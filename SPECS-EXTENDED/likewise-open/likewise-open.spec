...
Source0: 	https://github.com/vmware/likewise-open/%{name}-%{version}.tar.gz
Patch0:         likewise-open-openssl-1.1.1.patch
Patch1:         likewise-open-openssl-1.1.1-FixV2.patch
Patch2:         mariner_distro_support.patch
Patch3:         dcerpc-fix-mismatched-dealloc.patch
Patch4:         lwadvapi-update-asn1-mismatch-index-error.patch

BuildRequires:  pam-devel pam
...
Requires:       util-linux util-linux-devel
Requires:       ncurses ncurses-devel
Requires:       popt popt-devel
Requires:       flex
Requires:       libtool
...
# Patch for undefined symbol errors
# when loading libraries
# Issue seen with Mariner builds
sed -i 's/lsaserverapi sqlite3/lsaserverapi lwadvapi sqlite3/' lsass/server/auth-providers/ad-open-provider/MakeKitBuild
sed -i 's/lsacommon lsaserverapi/lsacommon lwadvapi lwbase lsaserverapi/' lsass/server/auth-providers/local-provider/MakeKitBuild
sed -i 's/lsacommon lsapstore/lsacommon lwadvapi lwbase lsapstore/' lsass/server/auth-providers/vmdir-provider/MakeKitBuild
sed -i 's/lwadvapi /lwadvapi lwbase /' centutils/libcentutils/MakeKitBuild
sed -i 's/rsutils lsacommon/rsutils lwadvapi lsacommon/' lsass/pstore/MakeKitBuild
sed -i 's/lwbase dcerpc/lwbase lwadvapi dcerpc/' lsass/client/rpc/lsa/MakeKitBuild
sed -i 's/regclient rsutils/regclient lwbase lwmsg rsutils/' lwreg/shell/MakeKitBuild
sed -i 's/regcommon lwmsg/regcommon lwbase lwmsg/' lwreg/client/MakeKitBuild
sed -i 's/uuid lwbase/uuid lwmsg lwbase/' lwreg/server/providers/sqlite/MakeKitBuild
sed -i 's/lwbase sqlite3/lwbase lwadvapi lwiocommon lwioshareinfo sqlite3/' lsass/server/store/samdb/MakeKitBuild
sed -i 's/dcerpc lwbase/dcerpc lwadvapi lwiocommon lwioshareinfo lwbase/' srvsvc/client/MakeKitBuild
...
# when loading libraries.
# Issue seen with Mariner builds
LDFLAGS="-lssl -lcrypto -lkrb5 -lkrb5support -lk5crypto -lgssapi_krb5 -lcom_err -lresolv -lldap -lldap_r -llber -lsasl2 -ldl -luuid"

export CFLAGS="-Wno-error=unused-but-set-variable -Wno-error=implicit-function-declaration -Wno-error=sizeof-pointer-memaccess -Wno-error=unused-local-typedefs -Wno-error=pointer-sign -Wno-error=address -Wno-unused-but-set-variable -Wno-unused-const-variable -Wno-misleading-indentation -Wno-error=format-overflow -Wno-error=format-truncation -Wno-address-of-packed-member -Wno-stringop-overflow"
...
%changelog
*   Mon Feb 01 2021 Suresh Babu Chalamalasetty <schalam@microsoft.com> 6.2.11.13-5
-   Initial version CBL-Mariner import from Photon
